...
A process supervisor (*s6-supervise*, but the reasoning holds for any of them)
...
```
s6-supervise                 (outside, the real supervisor: restart policy, logs, ./down …)
└─ 66-ns  (invoker)          the process s6-supervise forked and watches
...
- the **invoker** mirrors pid 1: it watches pid 1 and exits with pid 1's code, so
  *s6-supervise* sees a normal child exit and applies its restart policy as usual.

*66-ns* adds **no** supervision policy of its own: restart, `./down`, readiness,
timestamps, logs all remain *s6-supervise*'s job. *66-ns* only transports
life, death and signals faithfully.

### The two directions of transparency

**Signals go down.** A signal sent to the *66-ns* invoker (by *s6-supervise*,
...
re-exits with the same code, and *s6-supervise* records it. The status the

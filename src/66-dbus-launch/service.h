...
#define DBS_ENVIRONMENTFILE "100-dbus"
...
struct service_s {
	...
	char exec[SS_MAX_SERVICE_NAME + 1] ;
	char user[SS_MAX_SERVICE_NAME + 1] ;
	size_t id ;
	uint8_t pending ;
	UT_hash_handle hh ;
} ;

#define SERVICE_ZERO { {0}, {0}, {0}, 0, DBS_SERVICE_DELETE, NULL }
...
extern int service_environ_filepath(char *store, launcher_t *launcher) ;
...
extern void service_sync_dbus_list(launcher_t *launcher) ;

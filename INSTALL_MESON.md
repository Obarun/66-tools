...
- `oblibs` version `0.3.3.0` or later: [git.obarun.org/Obarun/oblibs](https://git.obarun.org/Obarun/oblibs).

- `66` version `0.8.1.0` or later: [git.obarun.org/Obarun/oblibs](https://git.obarun.org/Obarun/oblibs).
...
- `enable-dbus`: Enable 66-dbus-broker-launch with support for basu or elogind (choices: `disabled`, `basu`, `elogind`; default: `disabled`).
...
- `with-doc`: Build and install man pages and HTML documentation (default: true).
...
To reduce runtime dependencies without fully static executables, use `enable-static-deps` with `enable-static=true` to link dependencies (e.g., `skalibs`, `66`) statically.
...
- If `enable-static-deps` is enabled without `enable-static`, the build will fail to ensure static lib is available.
